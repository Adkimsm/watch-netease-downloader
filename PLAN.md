...
8. **灰色歌曲解锁**(2.0 起):网易云取不到可用直链时,改用第三方音源(酷我 / 酷狗)。
   **下载与在线播放各自一个开关**;可选的 `X-Real-IP` 地区解锁默认关闭。
   不引入代理,也不伪造网易云元数据 —— 置灰由本 App 自己的 `SongState` 决定。见 §15 的 D15–D19。
...
| 非 VIP 取不到 url | 置 MISSING_URL,UI 汇总跳过,不阻塞同步;解锁开关打开时先试第三方音源 |
| 第三方音源端点失效/改版 | 音源藏在 `SourceProvider` 接口后,新增或替换只改一个文件;失败一律降级为 MISSING_URL |
| 匹配到错误版本(Live/翻唱/伴奏) | 打分器按标题 + 歌手 + 时长三维,时长差 >20s 直接判 0;单测用真实响应钉住 |
| 音源限流/反爬(实测酷狗会偶发空 lists) | 5s 超时、并发 2、单轮上限 200 首;酷我优先兜底;失败不缓存,下轮再试 |
...
- [x] **Phase 13**:灰色歌曲解锁(第三方音源替换 + 地区解锁)—— 新增 `net/unlock/`
  (音源契约 / 可注入 HTTP + Range 探活 / 酷我 + 酷狗 / 候选打分 / 匹配器 / 解析器);
  下载与在线播放**各自独立开关**、音源开关、`X-Real-IP` 开关;DB v2→v3 增 `song.source`;
  曲目行显示来源徽标,播放开关打开时 MISSING_URL 的歌改为可点并显示「第三方音源」;
  同步预览新增「其中第三方音源 N 首」。新增 63 个单测(223→286),并用 5 处变异确认
  新用例不是空转;两个音源的端到端 live 测试实测通过。
- [x] **Phase 14**:离线删除排队 —— DB v3→v4 新增 `pending_removal` /
  `pending_removal_playlist` 两表;`library/PendingRemovalPlan.kt` 纯函数(要不要排队 / 合并 /
  执行计划 / 回写剩余 / 剪掉已消失的歌单);`SongRemover.removeOffline()`(本地当场删)与
  `flushPending()`(联网后统一执行,复用 `remove()` 的写后读回校验);`pendingRemovals`
  可观察队列跨 VM 与联网回调;`SyncEngine` **先执行队列再算差量** +
  `planDownloadCandidates` 排除排队歌;网络恢复自动执行(仅进程存活期)+ 同步前兜底;
  曲目行「待删除」徽标、首页「待删除 N 首 · 立即执行」、设置页清空、二级菜单取消。
  新增 21 个单测(296→317);`compileDebugKotlin` / `testDebugUnitTest` / `assembleDebug` 全过。
## 14. 实施纪要(避坑)
...
| D15 | 解锁范围 | 无版权 **与** VIP/仅试听(`freeTrialInfo` 非空)都替换 | 用户选定「全部」;判定与同步层原有的 MISSING_URL 逐字一致 |
| D16 | 音源 | 酷我 + 酷狗,酷我优先;不做 QQ / 咪咕 | 2026-09 实测只有这两个免登录可用;顺序按稳定性 |
| D17 | 解锁开关 | **下载**与**在线播放**各自一个开关,默认都开 | 串流不落盘、可即时反悔;下载会产出永久文件,后果不对等 |
...
- 离线删除期间,歌**仍在远端歌单里**(只是本地文件没了),曲目行标「待删除」。这是
  "没网也要能删"的代价:能删的只有本地那一半。队列没跑完之前,这首歌的歌单归属以服务端为准。
- 离线删除的撤销是"取消排队"(纯本地,没网可用),不是把远端加回去 —— 那时远端本来就
  一个字都没发出去。已执行后的撤销仍然走 `SongRemover.undo`。
...
- **离线删除有三条必须同时成立的不变量,少一条就会"歌又回来了"**:
  1. 排队的远端删除**必须在 `fetchUserPlaylists` 之前**执行 —— 顺序反了的话,拉回来的歌单
     还是"没删",差量立刻把这首歌算成待下载;
  2. 待下载集合必须**显式排除排队中的歌**(`planDownloadCandidates`)。只靠第 1 条不够:
     队列执行失败时远端确实还留着,单看"本地缺文件"就会把它下回来;
  3. 排队时**不能删本地的 `playlist_song` 关联**。关联一删,这首歌在本地就不再属于任何
     歌单,第 2 条也救不回来 —— 关联必须等远端真的删成功之后由 `remove()` 清理。
- **`getColumnIndexOrThrow` 在 LEFT JOIN 上要按别名取**:「只有红心、没有歌单目标」的条目
  在 `pending_removal_playlist` 侧整行为 NULL,读 `playlistId` 前必须判 `isNull`,否则拿到
  一个 0 并当成"歌单 0"去删。
- **不要用 `ON CONFLICT DO UPDATE`**:minSdk 29(Android 10)自带 SQLite 3.22 不支持
  UPSERT,真机上会直接抛语法错。合并策略放在纯函数里算好、再整条 `INSERT OR REPLACE` 写回,
  顺便让"覆盖"天然幂等(中途被杀重跑不会多删少删)。
- **撤销的语义随执行状态而变**:已执行的删除要调远端加回去,排队中的删除只能取消排队。
  用 `RemoveReport.queued` 区分,而不是看 `outcome` —— 两者 outcome 一模一样。
  同理 `queued` 不算 `hasFailure`,否则结果条会给"重试"而用户要的是"撤销"。

## 17. 灰色歌曲解锁:实测与踩坑(2026-09-25)

- **免登录还能用的音源只剩酷我与酷狗**。QQ 的 `vkey.GetVkeyServer` 对免费歌也返回
  `purl: ""`(旧 `fcg_music_express_mobile3.fcg` 直接 `104003`);咪咕 `listenSong.do`
  恒回「暂不提供试听地址」(要登录 userId),`getPlayInfo` 已变成 SPA HTML。
  照抄参考实现的 provider 列表会白写三个。
- **酷狗 `trackercdn.kugou.com/i/v2/` 已失效**:复测恒返回 `status: 2` 且无 `url`,
  改走 `m.kugou.com/app/i/getSongInfo.php?cmd=playInfo`(返回 url/fileSize/bitRate)。
- **酷我搜索返回的不是 JSON**:单引号、键不带引号,条目里还嵌着 `audiobookpayinfo:{...}`。
  按 `},{` 硬切会把条目切碎,必须做深度感知的顶层对象切分。
- **酷狗搜索会偶发返回 200 + 空 lists**(限流/反爬):按"这个音源这轮没戏"处理即可,
  所以酷我排在前面兜底。
- **不能取搜索结果第一条**:酷狗搜「晴天 周杰伦」的首条是「晴天 (Live)」134s,目标是
  269s 的原版 —— 盲取会安静地播错版本。必须打分 + 时长闸门。
- **Kotlin 的块注释会嵌套**:注释里写 `audio/*` 会打开一个嵌套注释,把后面整个文件吞掉,
  报错却是文件末尾的 "Unclosed comment"。注释里别写 `/*`。
- **`VAR=1 cmd && next` 不会把变量传给 `next`**:
...
- **酷我 `anti.s` 对受限歌曲返回固定 11 秒提示音**:实测《江南》《晴天》《小苹果》
  都是 181521 字节(128kbps 下约 11.3 秒),内容是「仅在酷我音乐手机端可播放」,
  不是这首歌。换 `br=320kmp3` 和 `nmobi` 的 `convert_url_with_sign` 也一样
